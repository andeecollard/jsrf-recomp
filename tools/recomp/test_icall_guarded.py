...
def test_a_recorded_site_is_guarded_with_the_generic_path_as_fallback():
    ...
    assert f"if (_icall_target == 0x{A:08X}u) {{ RECOMP_ICALL_GUARD_HIT(); RECOMP_ABI_CALL_G(0x{A:08X}u, sub_{A:08X}); }}" in c, c
    assert f"RECOMP_ABI_CALL_G(0x{B:08X}u, sub_{B:08X})" in c, c
...
def test_an_unrecorded_site_names_itself_and_nothing_else():
    ...
    assert "RECOMP_ABI_CALL_G" not in c, c
    assert "_icall_esp = g_esp" in c, c
    assert c.count("{") == c.count("}"), c
