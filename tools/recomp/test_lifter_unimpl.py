...
end-of-run tally and eight of which did not. JSRF's gen tree carries 122 of
...
three properties:
  ...
  3. instructions the lifter deliberately ignores -- the cache hints and the
     privileged port/cache instructions -- do NOT carry the marker. Those are
     decisions, written into the generated C as such, and a marker on them
     would make the runtime report count decisions as omissions.
...
class UnimplementedMarkerTest(unittest.TestCase):
    ...
    def test_ignored_privileged_has_no_marker(self):
        # Negative control: `out dx, al` is deliberately ignored and says so
        # with its own token.
        lifter, out = _lift("out", "dx, al", [_reg("dx"), _reg("al")])
        self.assertIn("RECOMP-IGNORED-PRIV", out)
        self.assertNotIn("RECOMP_UNIMPL", out)
        self.assertEqual(lifter.unimplemented, {})
