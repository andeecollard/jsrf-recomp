...
class BitTestCarryTest(unittest.TestCase):
    ...
    def test_bit_string_form_also_answers_from_the_snapshot(self):
        ...
        generated = "\n".join(lifted)
        self.assertIn("if (_cf)", generated)
        # The property is the ORDER -- the carry is captured before bts
        # overwrites the bit it reports. Upstream locates the write by its
        # `= (MEM32` spelling; this tree routes every memory write through the
        # traced RECOMP_MEM_WRITE32 macro instead, so the write is found by the
        # statement that performs it rather than by an assignment that is never
        # emitted here.
        self.assertLess(generated.index("_cf = "),
                        generated.index("RECOMP_MEM_WRITE32"))
