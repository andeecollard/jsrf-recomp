...
int main(void) {
    ...
    for (unsigned i = 0; i < N; i++) {
        for (unsigned j = 0; j < N; j++) {
            /* The instruction writes its result to the destination, and
               publishes it into the _fa/_fas snapshot pair next to that
               write; the jcc that follows reads the snapshot, not the
               destination, so that a mov or a pop in between cannot be
