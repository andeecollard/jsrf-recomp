...
Ported verbatim from upstream PR #120 (NoRain211, sp00nznet/xboxrecomp, MIT).
The PR's lifter change is not needed here: this tree already casts the result
to the operand width (`_sf_cast` in _make_condition), which is the same fix
reached independently. The test is taken as a regression guard -- with
`_sf_cast` forced back to `(int32_t)` it fails, so it does test the rule.
"""
