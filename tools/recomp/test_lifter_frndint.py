...
floor() and ceil() are `_ctrlfp(RC=down/up); _frnd(); _ctrlfp(old)`, and _frnd
(0x17EED5 in JSRF) is a bare frndint, so both of them rounded to nearest: floor
of 2.7 was 3. JSRF's skeletal animation takes its keyframe pair from floor and
ceil of the frame, so a looping animation could read a key past the end of
its table once per loop and draw a frame of garbage bones -- the cutscene that
vanished once a second.
