...
Ported from upstream PR #110 (NoRain211, sp00nznet/xboxrecomp, MIT); the last
test differs because this tree keeps the UNRESOLVED FLAGS marker on a
fallback, so the census and audit_unresolved_flags.py still count it.
...
In JSRF's tree of 22 Sep 2026 all five LOOP sites sit inside jump tables the
disassembler decoded as code (experiments/lifter_flags/census.md), so this
changes no live code there; it is here because the lift was wrong.
"""
...
def test_loop_preserves_the_comparison_for_a_later_branch():
    ...
    for opcode in (b"\xE2", b"\xE1", b"\xE0"):
        ...
        assert "UNRESOLVED FLAGS" not in code, code


def test_loope_without_a_tracked_setter_keeps_the_marked_fallback():
    ...
    assert "ecx -= 1;" in code, code
    assert "(ecx != 0) && (_flags /* loope - UNRESOLVED FLAGS */)" in code, code


def test_a_bare_lifter_lifts_loop_too():
    # The standalone path (no block context) must not fall back to _flags
    # for plain LOOP either.
    from tools.recomp.disasm import Instruction, Operand
    from tools.recomp.lifter import Lifter
    insn = Instruction(0, 2, "loop", "0x401000", "")
    insn.operands = [Operand(type="imm", imm=0x401000)]
    insn.jump_target = 0x401000
    out = "\n".join(Lifter().lift_instruction(insn))
    assert "ecx -= 1;" in out, out
    assert "ecx != 0" in out, out
    assert "UNRESOLVED" not in out, out
...
if __name__ == "__main__":
    ...
    test_loope_without_a_tracked_setter_keeps_the_marked_fallback()
    test_a_bare_lifter_lifts_loop_too()
    print("ok")
