...
/* Guarded-arm counters, defined here whether or not the feature is on: the
 * generated code references them wherever a site was guarded, and a build
 * without feedback must still link. */
...
/* Per-site targets. Open addressing over 32K slots (JSRF has 7,401 call
 * sites); each slot holds up to SITE_MAX distinct targets and a saturation
 * mark beyond that, which the lifter reads as "do not guard this one". */
