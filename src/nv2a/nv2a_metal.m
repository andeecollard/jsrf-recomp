...
@"#include <metal_stdlib>\n"
 ...
 "struct Params { uint width,height,dither,untextured,combiner_count,texture_mask,add_specular,alpha_test,alpha_ref,modulate,blend,depth_test,depth_write;"
 " uint tw[4],th[4],pitch[4],linear[4],rgba8[4],dxt1[4],repeat[4],levels[4],min_filter[4]; float lod_bias[4];"
 " uint color_icw[8]; uint alpha_icw[8]; };\n"
 ...
 " uint at=base+(s.rgba8[u]?4*morton(uint(p.x),uint(p.y),w,h):s.dxt1[u]?uint(p.y/4)*pitch+uint(p.x/4)*8:uint(p.y)*pitch+uint(p.x)*2);\n"
 ...
 " base+=s.dxt1[u]?((w+3)/4)*((h+3)/4)*8:s.rgba8[u]?w*h*4:pitch*h;w=max(1u,w/2);h=max(1u,h/2);pitch=s.dxt1[u]?((w+3)/4)*8:s.rgba8[u]?w*4:pitch;}"
 " if(s.rgba8[u]||s.dxt1[u]){if(s.repeat[u])uv-=floor(uv);else uv=clamp(uv,float2(0),float2(1));uv*=float2(w,h);}"
 ...
 "fragment float4 fs(Out i [[stage_in]], float4 dst [[color(0),raster_order_group(0)]],"
 ...
 " if(s.combiner_count){float4 r[13];for(uint n=0;n<13;n++)r[n]=float4(0);r[4]=d0;r[5]=d1;r[8]=tex;"
 ...
 " r[12].a=(s.texture_mask&1)?r[8].a:1;for(uint stage=0;stage<s.combiner_count;stage++){float4 next;"
 ...
 " next[k]=clamp(a*b+cc*d,-1.0f,1.0f);}r[12]=next;}c=clamp(r[12]+(s.add_specular?float4(r[5].rgb,0):float4(0)),0.0f,1.0f);}" 
 " else if(!s.untextured){c=tex;c.a=clamp(d0.a,0.0f,1.0f)*(s.modulate?c.a:1);if(s.modulate)c.rgb*=max(float3(0),d0.rgb);}" 
 " if((s.alpha_test&&uint(clamp(c.a,0.0f,1.0f)*255+.5f)<=s.alpha_ref)||(s.depth_test&&i.p.z>dst.a))return dst;"
 " if(s.blend)c.rgb=c.rgb*c.a+dst.rgb*(1-c.a);"
 " if(s.dither){constexpr uint b[16]={0,8,2,10,12,4,14,6,3,11,1,9,15,7,13,5};int2 xy=int2(i.p.xy);"
 " float bias=(float(b[(xy.y&3)*4+(xy.x&3)])+.5f)/16-.5f;c.rgb+=bias/float3(31,63,31);}return float4(c.rgb,s.depth_write?i.p.z:dst.a);}\n";

static int initialize(void)
{
    if(attempted) return pipeline!=nil;
    attempted=1;device=MTLCreateSystemDefaultDevice();if(!device)return 0;
...
typedef struct{uint32_t width,height,dither,untextured,combiner_count,texture_mask,add_specular,alpha_test,alpha_ref,modulate,blend,depth_test,depth_write;
    uint32_t tw[4],th[4],pitch[4],linear[4],rgba8[4],dxt1[4],repeat[4],levels[4],min_filter[4];
    float lod_bias[4];
    uint32_t color_icw[8],alpha_icw[8];}Params;
...
{float ar=area(v[a][0],v[b][0],v[c][0]);if(!isfinite(ar)||ar==0)return;int front=(ar>0)==(s->front_cw!=0);
 ...
 const float(*vertices)[16][4],unsigned count,unsigned primitive)
{
 ...
 if(s->depth_test&&(!depth||s->depth_pitch<(uint64_t)s->clip_w*4||(uint64_t)s->depth_pitch*s->clip_h>depth_size))return reject("depth-bounds");
 ...
 if(!n){reject_reason=NULL;return 0;}for(unsigned i=0;i<count;i++){for(unsigned k=0;k<4;k++)if(!isfinite(vertices[i][0][k])||!isfinite(vertices[i][3][k])||!isfinite(vertices[i][4][k]))return reject("non-finite-vertex");
    for(unsigned u=0;u<4;u++)if(s->texture_mask&(1u<<u)){for(unsigned k=0;k<4;k++)if(!isfinite(vertices[i][9+u][k]))return reject("non-finite-vertex");if(vertices[i][9+u][3]<=0)return reject("texture-q");}}
 @autoreleasepool{if(!initialize())return reject("initialization");
  int use_depth=s->depth_test;uint8_t*next_depth=use_depth?depth:NULL;uint32_t next_depth_pitch=use_depth?s->depth_pitch:0;size_t next_depth_size=use_depth?depth_size:0;
  ...
  if(!surface_valid||!depth_valid||surface_target!=target||surface_width!=s->clip_w||surface_height!=s->clip_h||surface_pitch!=s->target_pitch||surface_target_size!=target_size||depth_target!=next_depth||depth_pitch!=next_depth_pitch||depth_target_size!=next_depth_size){
   if(!nv2a_metal_sync())return reject("surface-sync");MTLTextureDescriptor*td=[MTLTextureDescriptor texture2DDescriptorWithPixelFormat:MTLPixelFormatRGBA32Float width:s->clip_w height:s->clip_h mipmapped:NO];td.usage=MTLTextureUsageRenderTarget;td.storageMode=MTLStorageModeShared;surface=[device newTextureWithDescriptor:td];if(!surface)return reject("surface-allocation");
   surface_target=target;surface_target_size=target_size;surface_width=s->clip_w;surface_height=s->clip_h;surface_pitch=s->target_pitch;size_t pixels=(size_t)surface_width*surface_height;float*rgba=malloc(pixels*16);if(!rgba)return reject("upload-allocation");
   for(unsigned y=0;y<surface_height;y++)for(unsigned x=0;x<surface_width;x++){const uint8_t*p=target+(size_t)y*surface_pitch+x*2;unsigned c=p[0]|(unsigned)p[1]<<8;size_t at=((size_t)y*surface_width+x)*4;rgba[at]=(float)(c>>11)/31;rgba[at+1]=(float)((c>>5)&63)/63;rgba[at+2]=(float)(c&31)/31;if(s->depth_test){const uint8_t*z=depth+(size_t)y*s->depth_pitch+x*4;uint32_t q=(uint32_t)z[1]|(uint32_t)z[2]<<8|(uint32_t)z[3]<<16;rgba[at+3]=(float)q/16777215;}else rgba[at+3]=1;}
   [surface replaceRegion:MTLRegionMake2D(0,0,surface_width,surface_height) mipmapLevel:0 withBytes:rgba bytesPerRow:surface_width*16];free(rgba);surface_valid=depth_valid=1;surface_dirty=depth_dirty=0;depth_target=next_depth;depth_target_size=next_depth_size;depth_pitch=next_depth_pitch;}
