...
int d3d8_host_check_streams(const D3D8HostDrawCheck *c, const D3D8ExecDrawTextures *e)
{
    ...
    if (c->draw_kind >= 3u) return 1;     /* G75: DrawVerticesUP, Begin/End -- inline, the executor has no arrays to compare */
...
/* G51.1: only the seven words the before-draw handler reads are set; the rest
                                  ...
                                  uint32_t rt_size, uint32_t zs_data, uint32_t zs_size)
{
    ...
    s_slot[i].check.zs_data = zs_data; s_slot[i].check.zs_size = zs_size; s_slot[i].check.vs_handle = vs_handle;
    atomic_store_explicit(&s_slot[i].state, 2u, memory_order_release);
    atomic_fetch_add(&s_enq, 1);
    return i + 1u;
}
...
static void on_token_body(uint32_t parameter)
{
    ...
    if (s_slot[i].kind == 1) {
        ...
        if (d3d8_host_any_draw_mode()) {
            ...
            if (d3d8_host_2d_verify_take(s_slot[i].check.serial)) {   /* a verify flip's draw: compare it */
                static D3D8HostDrawCheck v;
                v = s_slot[i].check; v.verify = 1;
                d3d8_host_2d_post(&v, s_exec_source);
            }
        }
        ...
        d3d8_host_2d_pre(p->serial, p->vs_handle, p->rt_data, p->rt_format, p->rt_size, p->zs_data, p->zs_size);
    }
