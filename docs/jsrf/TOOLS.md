...
| `RECOMP_FLIP_PACE_TILL_JUMP=1` | With `RECOMP_FLIP_PACE=0`: flips paced until the chapter jump fires, free after it (G76). JSRF's title falls into its attract loop by frames, so an arm fast enough at the title loses the harness's START; this keeps the stage's free-play measurement unpaced. |
...
| `[D3D8-HOST-2D] ... left to the executor:` | The lift's refusal census, one count per reason. Since G75 the texture reasons name the mode and unit (`texture shader mode 6 (bump env) unit 1`) and the format byte (`texture format 0x12`). What the host never sees has no reason: compare `[D3D8-HOST] coverage`'s executor batches a flip with the flight record's `host` column (the swap's copy quad is 1 a flip). |
| `RECOMP_D3D8_CENSUS=1` | Every D3D entry point's calls, game and internal, every 600 swaps: which D3D function issues a draw the mirror does not see (G75 found the HUD's Begin/End this way). |
| `RECOMP_STAGE_MEM=0x19A258:0x240` | Dumps D3D's three 2D pass-through programs, which D3D writes at start-up (the XBE image holds zeros there); G75 read their fog outputs with `nv2a_vsh_parse`. |
| `RECOMP_FRAME_SPLIT=1` | Where a frame goes, per thread, over [STAGE]'s window (`[FRAME-SPLIT]` lines, G76): the title's thread (every game-called D3D entry point at its outermost call, D3D's fence wait `D3D_BlockOnTime`, the ring's `MakeRequestedSpace`, the state flush, the mirror's hooks, its kernel calls by ordinal; "game" is the rest of the wall time), other threads' D3D calls, the pusher (host tokens by kind; the FF registers, build, vertex fetch and host encode; the wait for a software method's acknowledgement), and the GPU's command-buffer coverage. `rest` less these is the push-buffer walk. Read-only; ~40 ns per timed boundary. `gametools/frame_split.py <run dir>` averages a harness run's free-play windows. Use it, not `sample`, which suspends the game. |
| `jsrf_d3d8_host_2d_test ffbench <n>` | What one GPU-unit FF build of an n-vertex strip costs, per-vertex fetch and `fetch_run`, in one process: how G76 found the G75 build rise without a game run. |
